...
async def up(db):
    ...
    # reference has a durable ordinal.  V55 did not record ordering; its only
    # stable inputs are timestamp and handle id, so use that historical order.
    await db.execute("ALTER TABLE operation_resource_refs RENAME TO operation_resource_refs_v059")
    ...
    await db.execute("""INSERT INTO operation_resource_refs(operation_id, handle_id, position, created_at)
        SELECT target.operation_id, target.handle_id, COUNT(prior.handle_id), target.created_at
          FROM operation_resource_refs_v059 AS target
          LEFT JOIN operation_resource_refs_v059 AS prior
    ...
    await db.execute("DROP TABLE operation_resource_refs_v059")
