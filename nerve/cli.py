...
def doctor_report(config, config_source: str = "", check_api: bool = False) -> str:
    ...
    if anthropic_required:
        if config.provider.is_bedrock:
            ...
            if config.lockdown:
                # The generic message names config.local.yaml, which a locked
                # instance never reads. The cause may also be upstream of the
                # key: a provider block kept only in config.yaml falls back to
                # the Anthropic default in lockdown.
                errors.append(
