...
PRODUCTIVITY_CRONS = [
    ...
    },
    {
        ...
        "prompt": (
            ...
            "1. Load and follow nerve-skill-development before authoring anything\n"
            "2. Recall recent behavior patterns and events\n"
            "3. Check existing skills and dependencies to avoid duplicates or copied shared instructions\n"
            "4. Look for repeated tool sequences, domain knowledge clusters, and reusable patterns\n"
            "5. For each candidate (max 2): create a task and propose a plan with a complete canonical SKILL.md, "
            "strict SemVer version, canonical metadata.nerve dependency declarations, resource/sidecar handling, "
            "and validation plus realistic forward-test evidence. Never propose legacy top-level Nerve fields or "
            "legacy requires/suggests dependencies\n\n"
    ...
    },
    {
        ...
        "prompt": (
            ...
            "1. Load and follow nerve-skill-development before authoring anything\n"
            "2. Load all skills with skill_get; prioritize skills that show Pending amendments\n"
            "3. Check every amendment against its evidence, deduplicate it, and either incorporate it or explain why it should be rejected\n"
            "4. Check accuracy (outdated paths, commands, URLs), completeness, trigger quality, and dependencies\n"
            "5. For each skill needing changes (max 3), preserve dependencies/resources/sidecars, bump its SemVer version, "
            "and prepare one complete canonical SKILL.md replacement with canonical metadata.nerve declarations "
            "and without the amendments section\n"
            "6. Create a task with source=\"skill-reviser\". Include the skill ID, exact skill_revision and amendments_revision "
            "shown by skill_get (or state that there were none), retained/changed resources, rejected amendments, and a validation/forward-test report\n"
            "7. Propose a plan containing that metadata plus the complete replacement SKILL.md. Never propose legacy "
            "top-level Nerve fields or legacy requires/suggests dependencies\n\n"
